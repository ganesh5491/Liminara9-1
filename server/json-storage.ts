...
export class JSONStorage implements IStorage {
  ...
  async getProducts(filters?: {
    ...
    if (filters?.categoryId) {
      products = products.filter(p => p.categoryId === filters.categoryId || p.category === filters.categoryId);
    }
  ...
  async getWishlistItems(userId: string): Promise<(WishlistItem & { product: Product })[]> {
    ...
    const userWishlistItems = Array.from(this.wishlistItems.values())
      ...
      return userWishlistItems.map(item => {
        const product = this.products.get(item.productId);
        if (!product) {
          // Instead of throwing, we'll return a placeholder or skip
          console.error(`Product not found for wishlist item: ${item.productId}`);
          return null;
        }
        return { ...item, product };
      }).filter(item => item !== null) as (WishlistItem & { product: Product })[];
  }
  ...
  async getOrder(id: string): Promise<(Order & { orderItems: (OrderItem & { product: Product })[] }) | undefined> {
    ...
    const items = Array.from(this.orderItems.values())
      ...
      .map(item => {
        const product = this.products.get(item.productId);
        if (!product) {
          console.error(`Product not found for order item: ${item.productId} in order ${order.id}`);
          return null;
        }
        return { ...item, product };
      }).filter(item => item !== null) as (OrderItem & { product: Product })[];
