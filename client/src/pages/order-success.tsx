...
export default function OrderSuccessPage() {
  ...
  if (error || !order || !isValidOrder(order)) {
    // If we're still loading or retrying, don't show error yet
    if (isLoading) {
      return (
        <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center">
          <div className="text-center">
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-rose-100 flex items-center justify-center">
        <Card className="max-w-md mx-4">
          <CardContent className="p-6 text-center">
            <h1 className="text-xl font-bold text-red-600 mb-2">Order Confirmed!</h1>
            <p className="text-gray-600 mb-4">
              Your order has been placed successfully (ID: {orderId}). 
              We're just having a small trouble displaying the receipt right now.
              {error && <span className="block text-xs mt-2 text-gray-400">Technical details: {(error as Error).message}</span>}
            </p>
            <div className="flex flex-col gap-2">
              <Button onClick={() => window.location.reload()} variant="outline">
                Retry Viewing Receipt
              </Button>
  ...
  const totalAmount = parsePrice(order.total);
              ...
              {order.orderItems?.map((item: any) => (
                <div key={item.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-gray-800">₹{parsePrice(item.price).toFixed(2)}</p>
