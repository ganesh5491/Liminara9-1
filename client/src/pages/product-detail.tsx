...
export default function ProductDetail() {
  ...
  const wishlistProductIds = useMemo(() => {
    ...
    return wishlistItems.map((item: any) => String(item.productId));
  }, [wishlistItems]);

  useEffect(() => {
    if (product?.id) {
      const currentId = String(product.id);
      setIsWishlisted(wishlistProductIds.includes(currentId));
    }
  ...
  const handleBuyNow = () => {
    ...
    const buyNowItem = {
      ...
      total: parsePrice(product.price) * selectedQuantity
    };
