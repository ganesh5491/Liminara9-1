...
export default function CartModal({ isOpen, onClose }: CartModalProps) {
  ...
  const handleProceedToCheckout = () => {
    ...
    const checkoutItems = cartItems.map((item: any) => ({
      ...
      total: parsePrice(item.product?.price) * (item.qty || item.quantity || 1)
    }));
  ...
  const createOrderMutation = useMutation({
    mutationFn: async () => {
      ...
      const total = cartItems.reduce((sum: number, item) => {
        const product = item.product;
        const dealPrice = product?.isDeal && product?.dealPrice ? parsePrice(product.dealPrice) : null;
        const price = dealPrice !== null ? dealPrice : parsePrice(product.price);
  ...
  const total = cartItems.reduce((sum: number, item) => {
    ...
    const product = item.product;
    const dealPrice = product?.isDeal && product?.dealPrice ? parsePrice(product.dealPrice) : null;
    const price = dealPrice !== null ? dealPrice : parsePrice(product?.price || item.price);
