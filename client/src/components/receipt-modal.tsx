    ...
    return `
      ...
      <body>
          ...
          <tbody>
            ${orderData.orderItems?.map((item: any) => `
              <tr>
                ...
                <td>${item.quantity}</td>
                <td>₹${parsePrice(item.price).toLocaleString('en-IN')}</td>
                <td>₹${(parsePrice(item.price) * item.quantity).toLocaleString('en-IN')}</td>
              </tr>
        ...
        <div class="total-section">
          <div class="total-line">Subtotal: ₹${parsePrice(orderData.total).toLocaleString('en-IN')}</div>
          ${orderData.paymentMethod === 'cod' ? '<div class="total-line">COD Fee: ₹99</div>' : ''}
          <div class="grand-total">Grand Total: ₹${orderData.paymentMethod === 'cod' ? (parsePrice(orderData.total) + 99).toLocaleString('en-IN') : parsePrice(orderData.total).toLocaleString('en-IN')}</div>
        </div>
          ...
          <div className="border rounded-lg p-6 bg-gray-50 relative"
                  ...
                  <tbody>
                    {orderData.orderItems?.map((item: any, index: number) => (
                      <tr key={index} className="border-t">
                        ...
                        <td className="p-3">₹{parsePrice(item.price).toLocaleString('en-IN')}</td>
                        <td className="p-3">₹{(parsePrice(item.price) * item.quantity).toLocaleString('en-IN')}</td>
                      </tr>
                    )) || (
                        <tr>
            ...
            <div className="text-right relative z-10 bg-white bg-opacity-90 rounded-lg p-4">
              <div className="text-lg">
                Subtotal: <span className="font-semibold">₹{parsePrice(orderData.total).toLocaleString('en-IN')}</span>
              </div>
              ...
              <div className="text-xl font-bold text-darkBrown border-t border-gray-300 pt-2 mt-2">
                Grand Total: ₹{orderData.paymentMethod === 'cod'
                  ? (parsePrice(orderData.total) + 99).toLocaleString('en-IN')
                  : parsePrice(orderData.total).toLocaleString('en-IN')
                }
              </div>
            </div>
